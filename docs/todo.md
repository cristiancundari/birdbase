...
(D) #PROMEMORIA - Sistemare badge di presenza evento nel calendario che risulta nella data successiva {cm:2024-10-05}
(D) #PROMEMORIA - Impostare la priorità del promemoria a bassa di default {cm:2024-10-05}
(C) #PROMEMORIA - Sincronizzare il calendario con google {cm:2024-10-05}
(F) #PROMEMORIA - Impostare la data del promemoria che viene creato uguale alla data selezionata sul calendario
(D) #IMPOSTAZIONI - Rimuovere alexa e aggiungere google se necessario {cm:2024-10-05}
...
(C) #PROMEMORIA - Aspettare che si concluda l'operazione di inserimento nuovo promemoria utilizzando un loader sul pulsante {cm:2024-10-05}
