...
(D) #PROMEMORIA - Sistemare badge di presenza evento nel calendario che risulta nella data successiva {cm:2024-10-11}
(D) #PROMEMORIA - Impostare la priorità del promemoria a bassa di default {cm:2024-10-11}
(C) #PROMEMORIA - Sincronizzare il calendario con google {cm:2024-10-11}
(F) #PROMEMORIA - Impostare la data del promemoria che viene creato uguale alla data selezionata sul calendario
(D) #IMPOSTAZIONI - Rimuovere alexa e aggiungere google se necessario {cm:2024-10-11}
...
#GENERALE (A) Assicurarsi che la validazione errorri dei form lato client sia conforme a quella del server
...
#API (A) Rendere l'API utilizzabile solo da chi ne ha i permessi
#COVATE (A) Generare un errore quando si prova ad eliminare una covata con dei figli associati
#DATABASE (C) Sistemare l'azione di OnDelete e OnUpdate sullo schema prisma
(F) Gestire i morti {cm:2024-01-12}
#GENERALE (F) Verificare tutte le validazioni di dati lato client nei vari form dei modal (valutare se utilizzare zod "mantine-form-zod-resolver")
#SOGGETTO (C) Verificare durante la modifica del sesso di un soggetto se il soggetto è padre o madre di una covata e generare errore in caso affermativo
#COVATE (E) Modificare layout combobox in ModalAggiungiFiglio in modo che sia visibile la data di nascita, la gabbia e il preferito
(C) Aggiungere popover a SoggettoComp in modo da poter visualizzare eventuali note {cm:2024-01-26}

#PORTAFOGLIO (D) Mostrare nel report del portafoglio le opzioni che l'utente ha selezionato per generarlo
#GENERALE (D) Disattivare il submit di tutti i form quando si preme il tasto invio
#COVATE (D) Valutare di aggiungere la funzionalità che permette di aggiungere multipli figli a una covata
(C) Proteggere l'accesso ai bucket di supabase (al momento pubblici)
#EMAIL (C) Verificare un dominio su Resend.com e modificare l'invio di email in modo da inviarle all'utente effettivo invece che a "delivered@resend.dev" {cm:2024-10-11}
#EMAIL (C) Creare dei template HTML per le email
