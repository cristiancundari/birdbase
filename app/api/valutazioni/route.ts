...
export async function GET(request: NextRequest) {
  try {
    ...
    for (let mioSoggetto of mieiSoggetti) {
      const parenti = elaboraParenteleSoggetto(tuttiSoggetti, mioSoggetto, user);
    ...
    const risultatiGare = await prisma.iscrizione.findMany({
      where: { soggettoId: { in: Array.from(idSoggetti.values()) }, voto: { not: null } },
      include: { gara: true },
    });
    ...
    for (let soggetto of mieiSoggetti) {
      const gareSoggettoEParenti: (IscrizioniWithGara & { gradoParentela: number })[] = [];
      if (dictGare[soggetto.id] && dictGare[soggetto.id].length > 0) {
        gareSoggettoEParenti.push(...dictGare[soggetto.id].map((g) => ({ ...g, gradoParentela: 0 })));
      }
      for (let parente of dictSoggettiParenti[soggetto.id]) {
        if (dictGare[parente.soggetto.id] && dictGare[parente.soggetto.id].length > 0) {
          gareSoggettoEParenti.push(...dictGare[parente.soggetto.id].map((g) => ({ ...g, gradoParentela: parente.parentela!.grado })));
        }
      }
      const mediaPesataData = gareSoggettoEParenti.reduce(
        (accumulatore, elemento) => {
          const differenzaInAnni = differenceInDays(new Date(), elemento.gara.data) / 365.25;
      ...
      const valutazioneData = mediaPesataData.sommaPesi == 0 ? 0 : mediaPesataData.sommaValori / mediaPesataData.sommaPesi;
      ...
      const valutazioneParentela = mediaPesataParentela.sommaPesi == 0 ? 0 : mediaPesataParentela.sommaValori / mediaPesataParentela.sommaPesi;

      const valutazione = (valutazioneData * user.percentualeFormulaData + valutazioneParentela * (100 - user.percentualeFormulaData)) / 100;
      dictValutazioni[soggetto.id] = valutazione;
    }

    return NextResponse.json({ result: dictValutazioni, error: false }, { status: 200 });
  } catch (error: any) {
