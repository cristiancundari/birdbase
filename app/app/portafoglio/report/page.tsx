...
import { ActionIcon, Box, Center, Loader, Skeleton, Table, Title, Divider, Group, Text, Container, Button, Card } from "@mantine/core";
import { formatValuta, showNotification } from "@/lib/helper";
import { apiFetch } from "@/lib/apiFetch";
import { TransazioneWithCategoria } from "@/types/types";
import { IconArrowNarrowLeft, IconPrinter, IconReport } from "@tabler/icons-react";
...
function ReportPage() {
  ...
  const totale = transazioni.map((item) => item.prezzo).reduce((a, b) => a + b, 0);

  useEffect(() => {
    getResult(dataInizio, dataFine, tipologia);
  }, []);

  async function getResult(dataInizio: string | null, dataFine: string | null, tipologia: string | null) {
    setIsLoading(true);
    const result = await apiFetch.get(`/api/transazioni?dataInizio=${dataInizio}&dataFine=${dataFine}&tipologia=${tipologia}`);
    if (result.error) {
      ...
      <Container>
        <Group justify={"space-between"} p="xs">
          <ActionIcon
            ...
            data-testid="btn_indietro"
          >
            <IconArrowNarrowLeft style={{ width: "70%", height: "70%" }} stroke={1.5} />
          </ActionIcon>
